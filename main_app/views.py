...
from .models import Attendance, Session, Subject, CollegeProfile, SyllabusDocument, Staff, Department 
...
def login_page(request):
    ...
    context = {
        ...
        'recent_syllabus': SyllabusDocument.objects.select_related('course')[:5],
    }
...
def guest_portal(request):
    context = {
        'college_profile': CollegeProfile.objects.first(),
        'recent_syllabus': SyllabusDocument.objects.select_related('course')[:12],
        'page_title': 'Guest Portal'
    }
    return render(request, 'main_app/guest_portal.html', context)
