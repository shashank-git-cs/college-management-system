...
def student_syllabus_list(request):
    student = get_object_or_404(Student, admin=request.user)
    syllabi = SyllabusDocument.objects.filter(course=student.course).select_related('course')
    context = {
        'page_title': 'My Syllabus',
        'syllabi': syllabi,
        'course': student.course,
    }
    return render(request, 'student_template/student_syllabus_list.html', context)
