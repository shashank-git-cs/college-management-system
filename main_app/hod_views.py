        ...
        if form.is_valid():
            ...
            try:
                user = CustomUser.objects.create_user(
                    email=email, password=password, user_type=2, first_name=first_name, last_name=last_name, profile_pic=passport_url or 'home_real/front-main.jpg')
                user.gender = gender
...
def add_student(request):
    ...
    if request.method == 'POST':
        if student_form.is_valid():
            ...
            try:
                user = CustomUser.objects.create_user(
                    email=email, password=password, user_type=3, first_name=first_name, last_name=last_name, profile_pic=passport_url or 'home_real/front-main.jpg')
                user.gender = gender
                user.address = address
...
def manage_college_profile(request):
    ...
    if request.method == 'POST':
        ...
        if not college_name or not vision or not mission:
            messages.error(request, 'College name, vision and mission are required')
        else:
            if profile is None:
            ...
            profile.about = about
            profile.save()
            messages.success(request, 'College vision and mission updated successfully')
            return redirect(reverse('manage_college_profile'))

    context = {
        'page_title': 'Manage College Profile',
        'profile': profile,
    }
    return render(request, 'hod_template/manage_college_profile.html', context)


def manage_syllabus(request):
    syllabi = SyllabusDocument.objects.select_related('course').all()
    courses = Course.objects.all()
    if request.method == 'POST':
        ...
        if not title or syllabus_file is None:
            messages.error(request, 'Title and syllabus file are required')
        else:
    ...
    return render(request, 'hod_template/manage_syllabus.html', context)


def delete_syllabus(request, syllabus_id):
    syllabus = get_object_or_404(SyllabusDocument, id=syllabus_id)
    syllabus.delete()
