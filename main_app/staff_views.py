...
def staff_student_directory(request):
    staff = get_object_or_404(Staff, admin=request.user)
    query = request.GET.get('q', '').strip()
    students = Student.objects.filter(course=staff.course).select_related('admin', 'course', 'session')
    if query:
        students = students.filter(Q(admin__first_name__icontains=query) | Q(admin__last_name__icontains=query) | Q(admin__email__icontains=query)).distinct()
    context = {
    ...
    return render(request, 'staff_template/student_directory.html', context)


def staff_syllabus_list(request):
    staff = get_object_or_404(Staff, admin=request.user)
    syllabi = SyllabusDocument.objects.filter(course=staff.course).select_related('course')
    context = {
        'page_title': 'Course Syllabus',
        'syllabi': syllabi,
        'course': staff.course,
    }
    return render(request, 'staff_template/staff_syllabus_list.html', context)
